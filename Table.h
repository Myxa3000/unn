...
using namespace std;
int const max_teachers = 100;

//СОЗДАЕМ КЛАСС УЧИТЕЛЕЙ
...
class Teacher {
    public:
        ...
        string adress;
        double salary;
        bool savetofilet(ofstream& file);
        bool loadfromfilet(ifstream& file);
        void display();
};

//СОЗДАЕМ КЛАСС КАФЕДРЫ
...
class Department {
    public:
        ...
        void init();
        void clear();
        void addteacher(string name, string phone, string email, string adress, double salary);
        bool removeteacherbyname(string name);
        void updateteacher(int index, string name, string phone, string email, string adress, double salary);
...
// }
//
// void Department::removeteacherbyname(string name) {
//     for (int i = 0; i < teachercount; i++) {
